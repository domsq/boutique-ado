...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        """
        Handle the Stripe payment_intent.succeeded webhook event
        """
        ...
        if order_exists:
            return HttpResponse(
                    content=f'Webhook received: {event["type"]} | \
                            SUCCESS: Verified order already in DB',
                            status=200)
        else:
            order = None
            try:
    ...
    def handle_payment_intent_payment_failed(self, event):
        """
        Handle the Stripe payment_intent.payment_failed webhook event
        """
